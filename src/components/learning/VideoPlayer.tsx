...
const VideoPlayer: React.FC<VideoPlayerProps> = ({ 
    ...
    if (isYouTube && youtubePlayerRef.current && youtubeReady) {
      const checkTime = () => {
        if (youtubePlayerRef.current && isPlaying) {
          ...
          if (currentSegmentData && currentVideoTime >= currentSegmentData.endTime && !showQuiz && !currentSegmentData.completed) {
  ...
  const handleQuizSubmit = () => {
    ...
    // Only update problems answered count if this segment wasn't completed before
    if (!currentSegmentData.completed) {
  ...
  const handleNextSegment = () => {
    ...
    if (isCorrect) {
      ...
      setSegments(prev => prev.map((seg, index) => 
        index === currentSegment ? { ...seg, completed: true } : seg
      ));
